...
describeIf('Stage 3A pickup handoff PostgreSQL (wekonnek_stage3_test)', () => {
  ...
  beforeAll(async () => {
    ...
    const okRegression =
      ...
      (user === 'victor' || user === STAGE7_CURRENT_SCHEMA_REGRESSION_DATABASE || (user === STAGE8_CURRENT_SCHEMA_REGRESSION_DATABASE || user === STAGE9_CURRENT_SCHEMA_REGRESSION_DATABASE));
