...
export class SocialAuthProviderService {
  ...
  private callback(provider: Provider) {
    const base = (this.config.get<string>('PUBLIC_API_URL') || 'http://localhost:3000').replace(/\/$/, '');
    let url: URL;
    try { url = new URL(base); } catch { throw new BadRequestException('PUBLIC_API_URL must be an absolute URL'); }
    const local = ['localhost', '127.0.0.1', '::1'].includes(url.hostname);
    if (!local && url.protocol !== 'https:') throw new BadRequestException('PUBLIC_API_URL must use HTTPS outside local development');
    return `${url.origin}${url.pathname.replace(/\/$/, '')}/api/auth/oauth/${provider.toLowerCase()}/callback`;
  }
  ...
  private safe(config: any) { return { ...config, encryptedClientSecret: undefined, encryptedPrivateKey: undefined, clientSecretConfigured: Boolean(config.encryptedClientSecret), privateKeyConfigured: Boolean(config.encryptedPrivateKey), callbackUrl: this.callback(config.provider as Provider) }; }
  ...
  async update(providerInput: string, body: any, actorId: string) {
    ...
    for (const key of ['clientId', 'teamId', 'keyId', 'enabled']) if (body[key] !== undefined) data[key] = body[key];
  ...
  async oauthCredentials(providerInput: string) { const config = await this.active(providerInput); return { clientId: config.clientId!, clientSecret: config.encryptedClientSecret ? this.decrypt(config.encryptedClientSecret) : undefined, redirectUri: this.callback(config.provider as Provider) }; }
}
