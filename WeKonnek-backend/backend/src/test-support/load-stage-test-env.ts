/**
 * Load stage-specific dotenv, preferring Stage 8 current-schema regression
 * when WEKONNEK_CURRENT_SCHEMA_REGRESSION=1.
 *
 * Stage 8 is the current schema tip: regression mode always loads
 * `.env.stage8.regression.test` (wekonnek_stage8_regression_test).
 * Historical Stage 7 regression env remains available for legacy docs only.
 */
...
export function loadStageTestEnv(stageEnvFileName: string): boolean {
  if (isCurrentSchemaRegressionMode()) {
    if (!existsSync(STAGE8_REGRESSION_ENV)) {
      throw new Error(
        `WEKONNEK_CURRENT_SCHEMA_REGRESSION=1 but missing ${STAGE8_REGRESSION_ENV}`,
      );
    }
    loadEnv({ path: STAGE8_REGRESSION_ENV, override: true });
    return true;
  }
