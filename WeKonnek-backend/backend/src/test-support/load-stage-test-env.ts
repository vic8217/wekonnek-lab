...
function isStage15aDisposableUrl(url: string | undefined): boolean {
  if (!url || url.trim() === '') return false;
  try {
    const database = parseAcceptanceDatabaseUrl(url).database;
    return isStage15aAcceptanceDatabase(database);
  } catch {
    return false;
  }
}
...
/**
 * Stage15A acceptance routing is dedicated. It never falls through to
 * frozen Stage12–14 historical parents. A generic
 * WEKONNEK_ACCEPTANCE_DATABASE_URL wins only when it already names
 * wekonnek_stage15a_*.
 */
export function loadStage15aTestEnv(): boolean {
  ...
  const overrideUrl = overrideSnap.url;
  const incomingDatabaseUrl = process.env.DATABASE_URL;
  const overrideIs15a = isStage15aDisposableUrl(overrideUrl);
  const incomingIs15a = isStage15aDisposableUrl(incomingDatabaseUrl);

  if (!existsSync(stagePath) && !overrideIs15a && !incomingIs15a) {
    return false;
  }
  ...
  if (overrideIs15a && overrideUrl) {
    restoreAcceptanceOverrideEnv(overrideSnap);
    applyAcceptanceDatabaseOverride();
  } else if (isStage15aDisposableUrl(process.env.WEKONNEK_ACCEPTANCE_DATABASE_URL)) {
    applyAcceptanceDatabaseOverride();
  } else if (!isStage15aDisposableUrl(process.env.DATABASE_URL) && incomingIs15a && incomingDatabaseUrl) {
    process.env.DATABASE_URL = incomingDatabaseUrl;
  }

  if (!isStage15aDisposableUrl(process.env.DATABASE_URL)) {
    return false;
  }
