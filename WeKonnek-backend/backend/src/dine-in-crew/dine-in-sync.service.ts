...
export class DineInSyncService{
 ...
 async recordOrder(orderId:number,type:string){const order=await this.prisma.wkOrder.findUnique({where:{id:orderId},include:{orderItems:true}});if(!order||!['dine_in','in_store'].includes(order.orderType))return null;return this.record(order.shopId,type,order.id,{order:{id:order.id,orderCode:order.orderCode,status:order.status,tableNumber:order.tableNumber,totalAmount:Number(order.totalAmount),notes:order.notes,createdAt:order.createdAt,paymentMethod:order.paymentMethod,discountType:order.discountType,discountAmount:Number(order.discountAmount),discountDetails:order.discountDetails,items:order.orderItems.map(item=>({id:item.id,productName:item.productName,quantity:item.quantity,price:Number(item.price),subtotal:Number(item.subtotal),status:item.status||'preparing'}))}})}
