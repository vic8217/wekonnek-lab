...
export default function CoordinatorLoginPage() {
	...
	async function handleSubmit(event: React.FormEvent) {
		...
		try {
			const response = await fetch('/api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ identifier: coordinatorId.trim().toUpperCase(), password }) });
	...
	return <main className="grid min-h-screen bg-[#f7f9fc] lg:grid-cols-2">
		...
		<section className="flex items-center justify-center p-5 sm:p-10"><div className="w-full max-w-md rounded-[28px] border border-[#ccd8e9] bg-white p-8 shadow-[0_20px_45px_rgba(26,48,83,.15)]">
			...
			<form onSubmit={handleSubmit} className="mt-7 space-y-5">{error && <div role="alert" className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
				<label className="block text-xs font-black text-slate-600">COORDINATOR USER ID<div className="relative mt-2"><BadgeCheck className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input value={coordinatorId} onChange={event => setCoordinatorId(event.target.value.toUpperCase())} required autoComplete="username" placeholder="WKC-000001" className="h-13 w-full rounded-xl border border-[#ccd8e9] pl-12 pr-4 font-mono text-sm uppercase outline-none focus:border-[#075cff] focus:ring-2 focus:ring-blue-100" /></div></label>
				...
				<div className="text-right"><Link href="/coordinator/reset-password" className="text-xs font-bold text-[#075cff] hover:underline">Forgot password? Use an admin reset key</Link></div>
