...
type Lead = {
  ...
  city_municipality?: string; barangay?: string; submitted_at: string; assigned_coordinator_id?: string; assignment_status: 'assigned' | 'unassigned';
};
...
export default function MerchantOnboardingPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.message || 'Unable to load merchant leads');
      setLeads(body);
  ...
  const claim = async (id: number) => {
    ...
    toast.success('Merchant assigned to your coordinator account.'); await load(); setTab('assigned');
  };

  return <div className="w-full">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><h2 className="text-2xl font-black text-[#071d43]">Merchant Onboarding</h2><p className="mt-1 text-sm text-[#4d6385]">Unassigned website leads are shown only when their city belongs to your assigned coordinator zone.</p></div><button onClick={load} className="flex h-11 items-center justify-center gap-2 rounded-xl border border-[#ccd8e9] bg-white px-5 text-sm font-black text-[#075cff]"><RefreshCw size={17} /> Refresh</button></div>
