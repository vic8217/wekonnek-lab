...
import { ClipboardList, LayoutDashboard, LogOut, Megaphone, Menu, Star, WalletCards, X } from 'lucide-react';
...
function ProtectedCoordinatorLayout({ children }: { children: React.ReactNode }) {
	...
	return <div className="min-h-screen overflow-x-clip bg-[#f4f6f8] lg:pl-[232px]">
		<div className="fixed inset-y-0 left-0 z-40 hidden overflow-y-auto lg:block">{sidebar}</div>
		...
		<header className="sticky top-0 z-50 flex h-[76px] min-w-0 items-center justify-between gap-3 border-b border-[#d7e0ed] bg-white px-4 sm:px-5 lg:px-7"><div className="flex min-w-0 items-center gap-3"><button onClick={() => setMobileOpen(true)} aria-label="Open navigation menu" className="shrink-0 rounded-lg p-2 hover:bg-slate-100 lg:hidden"><Menu /></button><div className="min-w-0"><h1 className="truncate text-sm font-black">{pageTitle}</h1><p className="truncate text-xs text-slate-500">{user.firstName || 'Coordinator'}</p></div></div><div className="flex shrink-0 items-center gap-3 sm:gap-5"><NotificationInboxBell /><div className="hidden text-right sm:block"><p className="text-sm font-black">{name}</p><p className="text-xs text-slate-500">Zone Coordinator</p></div></div></header>
		<main className="min-w-0 overflow-x-hidden p-4 sm:p-5 lg:p-10">{children}</main>
	</div>;
}
