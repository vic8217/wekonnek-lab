...
export default function CategoryMarketplacePage() {
  ...
  return <div className="min-h-screen bg-white text-[#111827] xl:grid xl:grid-cols-[250px_minmax(0,1fr)]">
    ...
    <main className="min-w-0"><header className="bg-[#ff0719] px-5 py-4 text-white shadow-[0_12px_25px_rgba(255,7,25,.2)] lg:px-8"><div className="flex items-center gap-4"><Link href="/customer/dashboard" className="flex min-h-11 items-center gap-2 text-sm font-bold"><ArrowLeft size={20} /> Back to home</Link><span className={`flex size-11 items-center justify-center rounded-full bg-gradient-to-br ${categoryStyle.gradient} shadow-lg`}><CategoryIcon size={24} strokeWidth={1.9} /></span><div><h1 className="text-xl font-black">{config.name}</h1><p className="text-xs text-white/90">{config.description}</p></div></div><form action="/customer/search" className="relative mt-3"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={20} /><input name="q" placeholder={`Search ${config.name.toLowerCase()}, merchants, specialties...`} className="h-12 w-full rounded-xl bg-white pl-12 pr-12 text-sm text-slate-700 outline-none" /><Mic className="absolute right-4 top-1/2 -translate-y-1/2 text-red-600" size={19} /></form></header>
