...
> = {
  food: {
    name: "Food",
    icon: "🍔",
    description: "For ordering meals, drinks, and quick food discovery.",
    specialties: [
      "All",
      "Filipino",
      "Fast Food",
  ...
  restaurants: {
    name: "Restaurants",
    icon: "🍽️",
    description: "Reserve tables and discover memorable dining experiences.",
    specialties: [
      "All",
      "Asian",
      "Italian",
  ...
  groceries: {
    name: "Groceries",
    icon: "🛒",
    description: "Fresh produce, pantry essentials, and everyday needs.",
    specialties: [
      "All",
      "Fresh",
      "Organic",
  ...
  pharmacy: {
    name: "Pharmacy",
    icon: "💊",
    description: "Health essentials and trusted neighborhood pharmacies.",
    specialties: [
      "All",
      "Medicine",
      "Vitamins",
  ...
  services: {
    name: "Services",
    icon: "🔧",
    description: "Book reliable professionals for home and personal needs.",
    specialties: [
      "All",
      "Home Repair",
      "Cleaning",
  ...
  deals: {
    name: "Deals",
    icon: "🏷️",
    description: "Discover the best local promotions, vouchers, and savings.",
    specialties: [
      "All",
      "Food",
      "Shopping",
  ...
  events: {
    name: "Events",
    icon: "📅",
    description: "Find local events, activities, workshops, and experiences.",
    specialties: [
      "All",
      "Today",
      "Weekend",
  ...
  wellness: {
    name: "Wellness",
    icon: "✨",
    description: "Feel your best with fitness, beauty, spa, and self-care.",
    specialties: [
      "All",
      "Spa",
      "Fitness",
  ...
  express: {
    name: "Express",
    icon: "🚚",
    description: "Fast local pickup, courier, and delivery services.",
    specialties: [
      "All",
      "Same Day",
      "Documents",
  ...
  vouchers: {
    name: "Vouchers",
    icon: "🎟️",
    description: "Use rewards and vouchers at trusted local partners.",
    specialties: ["All", "Food", "Retail", "Services", "New", "Expiring Soon"],
    merchants: [
      "Rewards Hub",
      "Food Pass",
      "Shop Saver",
  ...
  "scan-discover": {
    name: "Scan & Discover",
    icon: "▦",
    description: "Scan store codes to open menus, rewards, and experiences.",
    specialties: ["All", "Menus", "Rewards", "Tables", "Stores", "Events"],
    merchants: [
      "QR Dining",
      "Smart Store",
      "Scan & Save",
  ...
  bazaar: {
    name: "Bazaar",
    icon: "🛍️",
    description: "Explore unique local products and community sellers.",
    specialties: ["All", "Handmade", "Fashion", "Home", "Gifts", "Local Finds"],
    merchants: [
      "Maker Market",
      "Local Finds",
      "Craft Corner",
...
// Bazaar never uses the demo specialty fallback: its Admin Category record is
// the system-wide source of truth for browsing and posting.
configs.bazaar.specialties = ["All"];
...
const photos = [
  "/assets/homepage-section-7/image-1.png",
  "/assets/homepage-section-7/image-2.png",
...
export default function CategoryMarketplacePage() {
  ...
  const config = {
    ...
    specialties: [
      "All",
      ...(managedCategory?.subCategories || []).map((item) => item.name),
    ],
  };
      ...
      <main className="min-w-0">
        <header className="bg-[#ff0719] px-5 py-4 text-white shadow-[0_12px_25px_rgba(255,7,25,.2)] lg:px-8">
          <div className="flex items-center gap-4">
            ...
            <span
              className={`flex size-11 items-center justify-center rounded-full bg-gradient-to-br ${categoryStyle.gradient} shadow-lg`}
            >
              <CategoryIcon size={24} strokeWidth={1.9} />
            </span>
          ...
          <form action="/customer/search" className="relative mt-3">
            <Search
            ...
            <input
              name="q"
              placeholder={`Search ${config.name.toLowerCase()}, merchants, specialties...`}
              className="h-12 w-full rounded-xl bg-white pl-12 pr-12 text-sm text-slate-700 outline-none"
            />
            <Mic
        ...
        <section className="border-b border-slate-200 bg-white px-4 py-3 lg:px-5">
          <div className="flex gap-2 overflow-x-auto pb-3 [scrollbar-color:#cbd5e1_transparent] [scrollbar-width:thin]">
            {config.specialties.map((item) => (
              <button
                key={item}
                onClick={() => setSpecialty(item)}
                className={`min-h-10 shrink-0 rounded-xl border px-4 text-sm font-semibold ${specialty === item ? "border-red-200 bg-red-50 text-red-600" : "border-slate-200 text-slate-600"}`}
              >
                {item}
              </button>
            ))}
          ...
          <div className="mt-2 flex gap-2 overflow-x-auto no-scrollbar">
            {["Popular", "Rating", "Nearby", "Offers", "Open now"].map(
              (item) => (
                <button
                  key={item}
                  onClick={() => setSort(item)}
                  className={`min-h-9 shrink-0 rounded-full px-4 text-xs font-bold ${sort === item ? "bg-red-50 text-red-600" : "bg-slate-100 text-slate-600"}`}
                >
                  {item}
                </button>
              ),
        ...
        <div className="grid min-h-[calc(100vh-190px)] xl:grid-cols-[390px_minmax(0,1fr)]">
              ...
              >
                <div className="relative size-24 shrink-0 overflow-hidden rounded-xl md:aspect-[4/3] md:h-auto md:w-full md:rounded-none xl:size-24 xl:rounded-xl">
                  <Image
                    ...
                    alt={row.name}
                    fill
                    sizes="(min-width: 768px) and (max-width: 1279px) 33vw, 96px"
                ...
                <div className="min-w-0 flex-1 md:p-3 xl:p-0">
                  <div className="flex justify-between gap-2">
                    <h2 className="truncate text-sm font-black">{row.name}</h2>
                    <Heart size={17} className="shrink-0 text-slate-400" />
                  </div>
                  <p className="mt-1 truncate text-xs text-slate-500">
                    {specialty === "All"
                      ? config.specialties[1] || config.name
                      : specialty}{" "}
                    · Local
                  </p>
                  <p className="mt-2 text-xs">
                    <span className="text-amber-500">★</span> {row.rating} ·{" "}
                    {row.distance}
                  </p>
                  ...
                  <span className="mt-2 inline-block rounded bg-red-50 px-2 py-1 text-[10px] font-bold text-red-600">
                    {row.offer} on selected items
                  </span>
                </div>
              </Link>
            ))}
          ...
          <section className="relative hidden overflow-hidden bg-[#e7f4ec] xl:block">
            <div className="absolute inset-0 opacity-30 [background-image:linear-gradient(35deg,transparent_45%,#93c5fd_46%,#93c5fd_49%,transparent_50%)] [background-size:140px_90px]" />
            <button className="absolute left-1/2 top-4 z-10 -translate-x-1/2 rounded-full bg-white px-5 py-3 text-xs font-bold shadow-lg">
              Search this area
            </button>
            {rows.slice(0, 7).map((row, index) => (
              ...
              >
                <span className="mx-auto flex size-9 items-center justify-center rounded-full border-4 border-white bg-[#ff0719] text-sm text-white shadow-lg">
                  {config.icon}
                </span>
                <span className="mt-1 block whitespace-nowrap rounded-full bg-white px-2 py-1 text-[9px] font-bold shadow">
                  {row.name}
                </span>
              </div>
            ))}
            <MapPin
              className="absolute left-1/2 top-1/2 text-blue-600"
              size={25}
            />
            <div className="absolute inset-x-4 bottom-4 rounded-2xl bg-white p-4 shadow-xl">
              <b className="text-sm">{config.name} specialties</b>
              <p className="mt-2 text-xs text-slate-500">
                {config.specialties.slice(1).join("  ·  ")}
              </p>
            </div>
            <div className="absolute bottom-24 right-5 overflow-hidden rounded-xl bg-white shadow-lg">
              <button className="block size-11 text-xl font-bold">+</button>
              <button className="block size-11 border-t text-xl font-bold">
                −
              </button>
            </div>
          </section>
