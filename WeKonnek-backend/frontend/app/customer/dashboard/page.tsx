...
const merchantCover = (merchant: Merchant, index: number) =>
  merchant.coverImageUrl ||
  merchant.logoUrl ||
  merchantFallbackImages[index % merchantFallbackImages.length];
...
export default function CustomerDashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<"delivery" | "dinein" | "pickup">(
    "delivery",
  );
  ...
  useEffect(() => {
    ...
    const fetchMerchants = async () => {
      try {
        ...
        const response = await merchantsApi.search(
          { limit: 10, page: 1 },
          controller.signal,
        );
        if (!active) return;
        ...
        try {
          const allMerchants = await merchantsApi.getAll(controller.signal);
          if (!active) return;
      ...
      <div className="xl:hidden bg-white min-h-screen pb-20">
        ...
        <div className="px-4 pt-2 pb-3">
          <p className="text-sm text-gray-500">
            Anything you want,{" "}
            <span className="font-semibold text-[#DB0002]">WeKonnek na!</span>
          </p>
        </div>
        ...
        <section className="pb-4">
          <div className="flex items-center justify-between px-4 mb-3">
            ...
            <Link
              href="/customer/deals"
              className="text-xs text-[#DB0002] font-semibold"
            >
              Explore All &gt;
            </Link>
          </div>
          <div className="flex gap-3 overflow-x-auto px-4 no-scrollbar">
            <div className="flex-shrink-0 w-48 bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl p-4 text-white relative overflow-hidden">
              ...
              <span className="inline-block bg-white/20 text-[9px] font-bold px-2 py-0.5 rounded-full mb-2">
                50% OFF
              </span>
        ...
        <section className="px-4 pb-4">
          <div className="flex items-center justify-between gap-2 bg-gray-50 rounded-2xl px-3 py-3 border border-gray-100">
            {trustBadges.map((b) => (
              ...
              >
                <div
                  className={`w-9 h-9 ${b.bg} ${b.color} rounded-full flex items-center justify-center mb-1`}
                >
                  {b.icon}
                </div>
          ...
          {merchantsLoading ? (
            <div className="px-4 flex gap-3">
              {[1, 2].map((i) => (
                <div
                  key={i}
                  className="w-56 flex-shrink-0 bg-gray-100 rounded-2xl h-48 animate-pulse"
                />
              ))}
            </div>
            ...
            <div className="flex gap-3 overflow-x-auto px-4 no-scrollbar">
              {featuredMerchants.map((merchant, index) => {
                ...
                const cover = merchantCover(merchant, index);
                return (
                  <Link
                    key={merchant.id}
                  ...
                  >
                    <div className="relative h-32 bg-gradient-to-br from-gray-800 to-gray-900">
                      ...
                      {merchant.logoUrl && merchant.logoUrl !== cover && (
                        <img
                    ...
                    <div className="p-3">
                      ...
                      <p className="text-[10px] text-gray-400 mt-0.5">
                        {interested.toLocaleString()} people interested
                      </p>
                ...
                dinein: (
                  ...
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M3 13h18M5 17h14a2 2 0 002-2v-2H3v2a2 2 0 002 2zM12 3v4m-4 0h8"
                    />
                  </svg>
                ),
                pickup: (
          ...
          {merchantsLoading ? (
            <div className="px-4 flex gap-3">
              {[1, 2].map((i) => (
                <div
                  key={i}
                  className="w-44 flex-shrink-0 bg-gray-100 rounded-2xl h-56 animate-pulse"
                />
              ))}
            </div>
            ...
            <div className="flex gap-3 overflow-x-auto px-4 no-scrollbar">
              {popularMerchants.map((merchant, idx) => {
                ...
                const cover = merchantCover(merchant, idx);
                return (
                  <Link
                    key={merchant.id}
                    href={`/merchants/${merchant.slug}`}
                  ...
                  >
                    <div className="relative">
                      ...
                      />
                      <span
                        className={`absolute top-2 left-2 ${badgeColors[idx % badgeColors.length]} text-white text-[9px] font-bold px-2 py-0.5 rounded-full`}
                      >
                    ...
                    <div className="p-3">
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className="text-[10px] text-gray-300 mx-0.5">
                          |
                        </span>
                        <span className="text-[10px] text-gray-500 truncate">
                          {merchant.category?.name || "Filipino"}
                        </span>
                      </div>
                    </div>
                  </Link>
          ...
          {merchantsLoading ? (
            <div className="px-4 flex gap-3">
              {[1, 2].map((i) => (
                <div
                  key={i}
                  className="w-44 flex-shrink-0 bg-gray-100 rounded-2xl h-52 animate-pulse"
                />
              ))}
            </div>
        ...
        {authUser ? (
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">
              Welcome back, {authUser.firstName || "User"}!
            </h1>
            <p className="text-gray-600">
              Manage your account, orders, and promotions
            </p>
          </div>
        ) : (
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600">
                Browse our services below — sign in anytime to order, book, and
                track.
              </p>
            </div>
            <Link
                ...
                <div className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center">
                  ...
                  >
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M7 4V2a1 1 0 011-1h8a1 1 0 011 1v2m0 0V1a1 1 0 011-1h2a1 1 0 011 1v3M7 4H5a1 1 0 00-1 1v16a1 1 0 001 1h14a1 1 0 001-1V5a1 1 0 00-1-1h-2M7 4h10"
                    />
                  </svg>
                </div>
                <div>
