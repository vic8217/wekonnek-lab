...
import { MapPinned, Plus, Search, X, Pencil, Trash2, Building2, Map, Upload, Download, FileText } from 'lucide-react';
...
type RegionImportRow = { regionCode: string; regionName: string; cityMunicipalityCode: string; cityMunicipalityName: string; localCouncilDistrict: string; areas: Area[] };
...
export default function ZoneManagementPage() {
  ...
  const [importRows, setImportRows] = useState<RegionImportRow[]>([]);
  ...
  const previewRegionPdf = async () => {
    if (!importFile) return toast.error('Choose a PDF file');
    setImporting(true);
    try {
      const form = new FormData(); form.append('file', importFile);
      const response = await api('/management-zones/region-import/preview', { method: 'POST', body: form });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.message || 'Unable to parse region PDF');
      setImportRows(result.rows || []);
      toast.success(`${result.count || 0} council districts found`);
    } catch (error) { toast.error(error instanceof Error ? error.message : 'Unable to parse region PDF'); }
    finally { setImporting(false); }
  };

  const downloadRegionTemplate = async () => {
    try {
      const response = await api('/management-zones/region-import/template');
      if (!response.ok) throw new Error('Unable to download template');
      const url = URL.createObjectURL(await response.blob());
      const link = document.createElement('a'); link.href = url; link.download = 'wekonnek-region-import-template.pdf'; link.click();
      URL.revokeObjectURL(url);
    } catch (error) { toast.error(error instanceof Error ? error.message : 'Unable to download template'); }
  };

  const importRegionRows = async () => {
    if (!importRows.length) return;
    setImporting(true);
    try {
      const response = await api('/management-zones/region-import', { method: 'POST', body: JSON.stringify({ rows: importRows }) });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.message || 'Unable to import region');
      toast.success(`${result.created} zones created${result.skipped ? ` · ${result.skipped} skipped` : ''}`);
      if (result.errors?.length) toast.error(`${result.errors.length} rows could not be imported`);
      setImportModal(false); setImportFile(null); setImportRows([]); await loadZones();
    } catch (error) { toast.error(error instanceof Error ? error.message : 'Unable to import region'); }
    finally { setImporting(false); }
  };
  ...
  return <div className="w-full space-y-6 pb-10">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="flex flex-wrap gap-3"><button onClick={() => { setImportModal(true); setImportFile(null); setImportRows([]); }} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#DB0002] bg-white px-5 text-sm font-bold text-[#DB0002] transition hover:bg-red-50"><Upload size={18}/>Upload region PDF</button><button onClick={openCreate} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#DB0002] px-5 text-sm font-bold text-white shadow-lg shadow-red-200 transition hover:-translate-y-0.5 hover:bg-red-700"><Plus size={18}/>Create zone</button></div>
    </div>
    ...
    {importModal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm"><div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-3xl bg-white shadow-2xl"><header className="flex items-start justify-between border-b border-slate-100 p-6"><div><h2 className="text-xl font-bold text-slate-900">Upload region mapping PDF</h2><p className="mt-1 text-sm text-slate-500">Import one coordinator zone per local council district. Review parsed coverage before saving.</p></div><button onClick={() => setImportModal(false)} className="rounded-full p-2 hover:bg-slate-100"><X size={20}/></button></header><div className="space-y-5 p-6"><button type="button" onClick={downloadRegionTemplate} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 text-sm font-bold text-blue-700"><Download size={17}/>Download PDF template</button><label className="flex cursor-pointer flex-col items-center rounded-2xl border-2 border-dashed border-slate-300 p-8 text-center transition hover:border-red-300 hover:bg-red-50/30"><FileText size={36} className="text-slate-400"/><span className="mt-3 font-bold text-slate-800">{importFile?.name || 'Choose a text-based region PDF'}</span><span className="mt-1 text-xs text-slate-500">PDF only · maximum 10 MB</span><input type="file" accept="application/pdf,.pdf" className="hidden" onChange={event => { setImportFile(event.target.files?.[0] || null); setImportRows([]); }}/></label><button onClick={previewRegionPdf} disabled={!importFile || importing} className="min-h-11 w-full rounded-xl bg-blue-600 px-5 text-sm font-bold text-white disabled:opacity-50">{importing && !importRows.length ? 'Reading PDF…' : 'Preview region mapping'}</button>{importRows.length > 0 && <div><div className="mb-3 flex items-center justify-between"><h3 className="font-bold text-slate-900">Parsed council districts</h3><span className="text-sm text-slate-500">{importRows.length} rows</span></div><div className="max-h-80 divide-y overflow-y-auto rounded-xl border border-slate-200">{importRows.map((row, index) => <div key={`${row.cityMunicipalityCode}-${row.localCouncilDistrict}-${index}`} className="p-3"><p className="font-semibold text-slate-900">{row.cityMunicipalityName} · {row.localCouncilDistrict}</p><p className="mt-1 text-xs text-blue-700">{row.areas.map(area => area.name).join(', ') || 'Whole district'}</p></div>)}</div></div>}</div><footer className="flex justify-end gap-3 border-t border-slate-100 p-5"><button onClick={() => setImportModal(false)} disabled={importing} className="min-h-11 rounded-xl border border-slate-200 px-5 text-sm font-bold">Cancel</button><button onClick={importRegionRows} disabled={!importRows.length || importing} className="min-h-11 rounded-xl bg-[#DB0002] px-6 text-sm font-bold text-white disabled:opacity-50">{importing && importRows.length ? 'Importing…' : `Import ${importRows.length || ''} zones`}</button></footer></div></div>}
      ...
      <div className="space-y-6 p-6">
        ...
        <section className="rounded-2xl border border-blue-100 bg-blue-50/40 p-4">
          ...
          {referenceLoading ? <div className="rounded-xl bg-white p-8 text-center text-sm text-slate-500">Loading Philippine locations…</div> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <Select label="Local council district" value={district} onChange={selectDistrict} options={availableDistricts.map((item) => ({ value: item, label: item }))} disabled={!localityCode}/>
