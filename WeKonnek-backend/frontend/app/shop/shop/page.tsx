    ...
    return (
      <article key={order.id} className="rounded-xl border p-4">
        <div className="flex items-start justify-between gap-4">
          <div><p className="font-black">{order.order_code || `Order #${order.id}`}</p><p className="text-sm text-gray-500">{order.customer_name || 'Customer'}{order.created_at ? ` · Opened ${new Date(order.created_at).toLocaleTimeString('en-PH', { hour: 'numeric', minute: '2-digit' })}` : ''}</p></div>
        ...
        </section> : <div className="mt-4 rounded-xl bg-gray-50 p-4"><p className="mb-3 text-xs font-black uppercase tracking-wide text-gray-500">Order summary</p><div className="space-y-2 text-sm"><div className="flex justify-between"><span>Subtotal</span><b>{money(subtotal)}</b></div>{draftReduction > 0 && <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-amber-950"><div className="flex justify-between font-bold"><span>Draft SC/PWD estimate</span><span>−{money(draftReduction)}</span></div><p className="mt-1 text-[10px]">{order.discount_details?.eligibleDiners} of {order.discount_details?.totalDiners} diners · Complete all required card details to request bill-out.</p></div>}<div className="flex justify-between border-t pt-3 text-base font-black"><span>{draftReduction > 0 ? 'Estimated amount due' : 'Total'}</span><span className="text-red-600">{money(draftReduction > 0 ? subtotal - draftReduction : Number(order.total_amount || 0))}</span></div></div></div>}
