...
export default function PostBazaarItemPage() {
  ...
  useEffect(() => {
    ...
    const verifyCustomer = async () => {
      ...
      if (!token) { router.replace('/auth/login?redirect=%2Fbazaar%2Fpost'); return; }
      ...
      if (!response.ok || (account?.userType ?? account?.role) !== 'customer') {
        router.replace('/auth/login?redirect=%2Fbazaar%2Fpost');
        return;
      }
      if (cancelled) return;
      setReady(true);
      merchantCategoriesApi.getBySlug('bazaar')
        .then(category => setSubcategories(category.subCategories || []))
        .catch(() => setCategoryError('Unable to load Bazaar subcategories. Please refresh and try again.'));
    };
    void verifyCustomer();
  ...
  const prepareListing = async (form: HTMLFormElement) => {
    ...
    try {
      ...
      photos.forEach(photo => uploadData.append('files', photo.file));
      ...
      const imageUrls = uploadBody.urls;
      const response = await fetch('/api/backend/bazaar-listings/drafts', {
        method: 'POST',
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-8">
    <form ref={formRef} onSubmit={submit} className="mx-auto max-w-2xl rounded-2xl border bg-white p-6 shadow-sm">
      <Link href="/customer/explore/bazaar" className="inline-flex items-center gap-2 text-sm font-bold text-slate-600"><ArrowLeft size={17} /> Back to Bazaar</Link>
      <h1 className="mt-5 text-3xl font-black">Post an Item</h1>
      <p className="mt-2 text-sm text-slate-500">Create a searchable Bazaar listing for ₱15 for 7 days.</p>

      <div className="mt-6 grid gap-5">
        <label className="text-sm font-bold">Item title<input name="title" required className="mt-2 w-full rounded-xl border px-4 py-3 font-normal" placeholder="What are you selling?" /></label>

        <label className="text-sm font-bold">Bazaar subcategory
          <select name="subCategoryId" required defaultValue="" disabled={!subcategories.length} className="mt-2 w-full rounded-xl border bg-white px-4 py-3 font-normal disabled:bg-slate-50">
        ...
        <label className="text-sm font-bold">Price<input name="price" required type="number" min="0" step="0.01" className="mt-2 w-full rounded-xl border px-4 py-3 font-normal" placeholder="₱0.00" /></label>
        <label className="text-sm font-bold">Description<textarea name="description" required rows={5} className="mt-2 w-full rounded-xl border px-4 py-3 font-normal" placeholder="Describe the condition, details, and pickup options." /></label>
        ...
        <button disabled={submitting} className="min-h-12 rounded-xl bg-red-600 font-black text-white transition hover:bg-red-700 disabled:cursor-wait disabled:opacity-60">{submitting ? 'Preparing your listing…' : 'Continue to Listing Payment'}</button>
      </div>
    </form>
