...
import { getTotalCartCount, onCartChange } from '@/lib/cart';
...
export default function Navigation() {
  ...
  // Keep the cart badge in sync with localStorage carts.
  useEffect(() => {
    const refresh = () => setCartCount(getTotalCartCount());
    refresh();
    return onCartChange(refresh);
  }, []);

  // Fetch search suggestions - defined early so it can be used in speech recognition
  ...
  return (
    <nav className="border-dotted border-[#165BB8]/30">
      {/* Top Navigation Bar - White Background for Mobile, Light Beige for Desktop */}
      <div className="bg-white lg:bg-[#FFFAF3]">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-3 lg:space-x-4">
              ...
              <Link
                ...
                className="hidden lg:inline-block min-w-[160px] px-4 py-2 border-2 border-[#DB0002] text-[#DB0002] rounded-md hover:bg-red-50 transition-colors font-medium text-center"
              >
                Become A Merchant
              </Link>
              <Link
                href="/auth/login"
                className="hidden lg:inline-block min-w-[160px] px-4 py-2 bg-[#DB0002] text-white rounded-md hover:bg-[#B80002] transition-colors font-medium text-center"
              >
                Sign In
              </Link>
              ...
              >
                ...
                />
                {cartCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] px-1 rounded-full bg-[#DB0002] text-white text-[10px] font-bold flex items-center justify-center">
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white border-b border-gray-200 shadow-lg animate-in slide-in-from-top">
          <div className="px-4 py-3 space-y-1">
            ...
            <Link
              href="/merchants/register"
              onClick={() => setMobileMenuOpen(false)}
              className="block px-4 py-3 rounded-lg text-[#DB0002] hover:bg-red-50 font-medium transition-colors"
            >
              Become A Merchant
            </Link>
            <Link
              href="/auth/login"
              onClick={() => setMobileMenuOpen(false)}
              className="block px-4 py-3 rounded-lg bg-[#DB0002] text-white hover:bg-[#B80002] font-medium text-center transition-colors"
            >
              Sign In
            </Link>
          </div>
      ...
      {mobileSearchOpen && (
        <div className="lg:hidden bg-[#165BB8] px-4 py-3 animate-in slide-in-from-top">
          <form onSubmit={(e) => { handleSearch(e); setMobileSearchOpen(false); }} className="space-y-2">
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="What can we help you with?"
              className="w-full px-4 py-3 rounded-lg outline-none bg-white text-gray-900 placeholder-gray-400"
              autoFocus
            />
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-lg outline-none bg-white text-gray-900 placeholder-gray-400"
              />
              <button
                type="submit"
                className="px-6 py-3 bg-[#DB0002] text-white rounded-lg font-medium hover:bg-[#B80002] transition-colors"
              >
                Search
              </button>
            </div>
      ...
      {/* Search Bar - Solid Blue Background */}
      <div className="bg-[#165BB8]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-4">
            {/* Search Input - Hidden on Mobile, Visible on Desktop */}
            <form onSubmit={handleSearch} className="hidden lg:block flex-1 relative">
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => {
                ...
                placeholder="What can we help you with?"
                className="w-full px-4 py-3 pr-24 rounded-lg outline-none bg-white text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-[#165BB8]/30"
                suppressHydrationWarning
              />
                    ...
                    <div className="py-2">
                      ...
                      {(searchQuery.trim() || locationQuery.trim()) && (
                        <div className="border-t border-gray-200 px-4 py-2">
                          ...
                          >
                            View all results for "{searchQuery || locationQuery}" →
                          </button>
                        </div>
                      )}
                    </div>
            ...
            <div className="relative flex-1 lg:flex-none">
              <input
                ...
                className="w-full px-4 py-3 pr-12 lg:pr-24 rounded-lg outline-none bg-white text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-[#165BB8]/30 lg:w-64"
                suppressHydrationWarning
              />
              {speechError && !isSearchListeningRef.current && (
                <div className="absolute top-full left-0 mt-1 px-3 py-2 bg-red-100 border border-red-300 rounded-md text-red-700 text-sm z-10 max-w-md">
                  {speechError}
                </div>
              )}
              <div className="absolute right-2 top-1/2 transform -translate-y-1/2 flex items-center gap-2">
