...
import { Check, Megaphone, PhilippinePeso, TrendingUp, X, Zap } from 'lucide-react';
...
const HIDDEN_KEY = 'bazaar_banner_hidden_until';
...
export default function BazaarSellerPromo() {
  ...
  const dismiss = () => {
    localStorage.setItem(HIDDEN_KEY, String(Date.now() + SEVEN_DAYS));
    setVisible(false);
    window.setTimeout(() => setPromo(null), 300);
  };
  const post = () => {
  ...
  return <>
    <section aria-label="Sell on WEKONNEK Bazaar" className={`relative m-4 overflow-hidden rounded-2xl border border-red-100 bg-gradient-to-br from-white via-white to-red-50 p-5 shadow-[0_12px_35px_rgba(185,28,28,.10)] transition-all duration-300 md:p-7 ${visible ? 'translate-y-0 opacity-100' : 'translate-y-2.5 opacity-0'}`}>
      <button type="button" onClick={dismiss} aria-label="Hide Bazaar seller promotion for seven days" className="absolute right-3 top-3 z-10 rounded-full p-2 text-slate-500 hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500"><X size={19} /></button>
